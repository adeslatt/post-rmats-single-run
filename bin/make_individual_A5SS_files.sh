...
allA5SSoriginalFiles="*.filtered.A5SS.MATS.JC.txt"
rmats="rMATS_"
A5SS="A5SS"
description="A5SS Events for DS-AML"
...
for file in $allA5SSoriginalFiles; do
    ...
    header=$track_name$rmats$name$underscore$A5SS$description$rmats$name$underscore$A5SS

    echo "name is $name"
    echo "header is $header"

    txt_filename=$name$period$A5SS$coordinates_txt_end
    bed_filename=$name$period$A5SS$coordinates_bed_end
    fasta_filename=$name$period$A5SS$coordinates_fasta_end
    orf_filename="${txt_filename%.txt}"
    cpat_output_filename=$name$period$A5SS$cpat_output
    cpat_error_filename=$name$period$A5SS$cpat_error
