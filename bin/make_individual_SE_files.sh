...
rmats="rMATS_"
SE="SE"
description="SE Events for DS-AML"
...
for file in $allSEoriginalFiles; do
    ...
    # old absolute path
    #    awk -f /Users/annedeslattesmays/Desktop/projects/post-rmats-single-run/bin/make_bed_se.awk $file >> $bed_filename
    # new relative path
    gawk -f "$SCRIPT_DIR/make_bed_se.awk" $file >> $bed_filename
    ...
    "$TRANSEQ_DIR/gotranseq" --sequence $orf_seqs_filename -o $orf_seqs_aa_filename -f F
